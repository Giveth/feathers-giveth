 ...
 *
 * @param {array} artifacts array of solcpiler|embark generated artifact for a solidity contract
...
function topicsFromArtifacts(artifacts, names) {
  return artifacts
    .reduce((accumulator, { abiDefinition, compilerOutput }) => {
      const abi = compilerOutput ? compilerOutput.abi : abiDefinition;
      if (!abi) return accumulator;

      return accumulator.concat(
        abi.filter(method => method.type === 'event' && names.includes(method.name)),
      );
    }, [])
    .reduce(
      (accumulator, event) =>
