...
export default function web3(options = {}) {
  return function () {
    ...
    if (!app.passport) {
      throw new Error('Can not find app.passport. Did you initialize feathers-authentication before feathers-authentication-web3?');
    }
...
export class Web3Challenger {
  constructor(app, options = {}) {
    ...
    this.challengeService = typeof options.challengeService === 'string' ? app.service(options.challengeService) : options.challengeService;
    ...
    if (!this.challengeService) {
      throw new Error('options.challengeService does not exist.\n\tMake sure you are passing a valid service path or service instance and it is initialized before feathers-authentication-web3.');
    }
  }
  ...
  getMessage(address, done) {
    ...
    this.challengeService.get(address)
      .then(message => {
  ...
  generateMessage(address, done) {
    ...
    this.challengeService.create({ address })
      .then(message => {
