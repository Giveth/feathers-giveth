...
module.exports = function uploadByImpactGraphService() {
  const app = this;
  ...
  app.use('/uploadByImpactGraph', express.static(app.get('uploads')));

  const service = new UploadByImpactGraphService();
  service.docs = {
    operations: {
      update: false,
      patch: false,
  ...
  app.use('/uploadByImpactGraph', multipartMiddleware.single('uri'), multipartTransfer, service);

  // Get our initialized service so that we can register hooks and filters
  app.service('uploadByImpactGraph').hooks(hooks);
};
